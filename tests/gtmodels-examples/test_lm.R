...
gt_model(models,
  digits = 3, var_labels = name_corr,
  summary_stats = c("rsquared", "adj_rsquared", "bic")
) |>
  cols_label(model1 = "(1)", model2 = "(2)", model3 = "(3)") |>
  fmt_number(
    columns = starts_with("model"),
    rows = "BIC",
    decimals = 0
  ) |>
...
gt_model(models,
  digits = 3, var_labels = name_corr, sig_thresh = c(0.05, 0.01, 0.001),
  summary_stats = c("bic", "deviance", "loglik", "pseudo_rsquared")
) |>
  cols_label(model1 = "(1)", model2 = "(2)", model3 = "(3)") |>
  rows_add(
    variable = "Null Deviance", model1 = model1$null.deviance,
